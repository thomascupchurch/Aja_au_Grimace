...
def fetch_tasks():
    ...
    def choose_colors(status: str, progress: int, start_dt, end_dt):
        ...
        if is_done:
            return {"color": "#ffffff", "color_progress": "#10b981"}
        if overdue or is_blocked:
            return {"color": "#ffffff", "color_progress": "#ef4444"}
        if is_on_hold:
            return {"color": "#ffffff", "color_progress": "#94a3b8"}
        if is_active_status or has_progress or in_window or (has_started and progress < 100 and not end_dt):
            return {"color": "#ffffff", "color_progress": "#FF8200"}
        return {"color": "#e5e7eb", "color_progress": "#9ca3af"}
